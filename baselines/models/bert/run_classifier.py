...
class CnliProcessor(DataProcessor):
    """Processor for the CNLI data set."""
    ...
    def get_train_examples(self, data_dir):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = tokenization.convert_to_unicode(line[3].strip())
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        lines = self._read_tsv(os.path.join(data_dir, "dev.csv"))
        examples = []
        for (i, line) in enumerate(lines):
            # if i == 0:
            #     continue
...
class CsnliProcessor(DataProcessor):
    """Processor for the C SNLI data set."""
...
class CmnliProcessor(DataProcessor):
    """Processor for the C MNLI data set."""
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        lines = self._read_tsv(os.path.join(data_dir, "dev-m.tsv"))
        examples = []
        for (i, line) in enumerate(lines):
            # if i == 0:
            #     continue
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        lines = self._read_tsv(os.path.join(data_dir, "dev-mm.tsv"))
        examples = []
        for (i, line) in enumerate(lines):
            # if i == 0:
            #     continue
